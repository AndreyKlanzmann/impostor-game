...
export default function HomePage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch("/api/rooms/create", {
        ...
        body: JSON.stringify({ hostName: name.trim(), mode, categories: selectedCategories }),
      })
  ...
  const handleJoin = async () => {
    if (!name.trim() || !code.trim()) { setError("Digite seu nome e o código da sala"); return }
    setLoading(true)
    setError("")
    try {
  ...
  const goBack = () => { setView("home"); setError(""); setCode("") }
          ...
          <motion.div key="home" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }}
            ...
            <div className="flex flex-col gap-3 w-full">
              ...
              <div className="flex gap-2">
                <Button size="lg" variant="secondary" className="flex-1 h-14" onClick={() => setView("create")}>
                  📱 Criar Sala
                </Button>
                <Button size="lg" variant="secondary" className="flex-1 h-14" onClick={() => setView("join")}>
                  🔗 Entrar
                </Button>
              </div>
            </div>
              ...
              <CardContent className="flex flex-col gap-4">
                <div>
                  ...
                  <Input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && handleCreate()}
                    placeholder="Ex: João" maxLength={20} autoFocus />
                </div>
                <div>
                ...
                <div>
                  <label className="text-sm text-muted-foreground mb-2 block">Categorias <span className="text-xs">(vazio = todas)</span></label>
                ...
                <Button onClick={handleCreate} disabled={loading || !name.trim()} className="w-full h-12">
                  {loading ? "Criando..." : "Criar Sala"}
                </Button>
                <Button variant="ghost" onClick={goBack} className="w-full">Voltar</Button>
              </CardContent>
            </Card>
          </motion.div>
        )}
              ...
              <CardContent className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <Input value={code} onChange={e => setCode(e.target.value.toUpperCase())} onKeyDown={e => e.key === "Enter" && handleJoin()}
                    placeholder="Ex: ABC123" maxLength={6} className="text-center text-2xl font-mono tracking-widest uppercase" />
                </div>
                {error && <p className="text-destructive text-sm text-center">{error}</p>}
                <Button onClick={handleJoin} disabled={loading || !name.trim() || code.length < 6} className="w-full h-12">
